...
import DeflatingHRC20 from '../build/contracts/DeflatingHRC20.json'
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    DTT = await deployContract(wallet, DeflatingHRC20, [expandTo18Decimals(10000)])

    // make a DTT<>WONE pair
    await fixture.factoryV2.createPair(DTT.address, wone.address)
    const pairAddress = await fixture.factoryV2.getPair(DTT.address, wone.address)
  ...
  async function addLiquidity(DTTAmount: BigNumber, WONEAmount: BigNumber) {
    await DTT.approve(router.address, MaxUint256)
    await router.addLiquidityONE(DTT.address, DTTAmount, DTTAmount, WONEAmount, wallet.address, MaxUint256, {
      ...overrides,
      value: WONEAmount
    })
  }

  it('removeLiquidityONESupportingFeeOnTransferTokens', async () => {
    const DTTAmount = expandTo18Decimals(1)
    const ONEAmount = expandTo18Decimals(4)
    await addLiquidity(DTTAmount, ONEAmount)

    const DTTInPair = await DTT.balanceOf(pair.address)
    const WONEInPair = await wone.balanceOf(pair.address)
    const liquidity = await pair.balanceOf(wallet.address)
    const totalSupply = await pair.totalSupply()
    const NaiveDTTExpected = DTTInPair.mul(liquidity).div(totalSupply)
    const WONEExpected = WONEInPair.mul(liquidity).div(totalSupply)

    await pair.approve(router.address, MaxUint256)
    await router.removeLiquidityONESupportingFeeOnTransferTokens(
      DTT.address,
      liquidity,
      NaiveDTTExpected,
      WONEExpected,
  ...
  it('removeLiquidityONEWithPermitSupportingFeeOnTransferTokens', async () => {
    const DTTAmount = expandTo18Decimals(1)
      .mul(100)
      .div(99)
    const ONEAmount = expandTo18Decimals(4)
    ...
    await pair.approve(router.address, MaxUint256)
    await router.removeLiquidityONEWithPermitSupportingFeeOnTransferTokens(
      DTT.address,
      liquidity,
      NaiveDTTExpected,
      WONEExpected,
  ...
  describe('swapExactTokensForTokensSupportingFeeOnTransferTokens', () => {
    ...
    // WONE -> DTT
    it('WONE -> DTT', async () => {
      await wone.deposit({ value: amountIn }) // mint WONE
      await wone.approve(router.address, MaxUint256)

      await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        amountIn,
        0,
        [wone.address, DTT.address],
        wallet.address,
        MaxUint256,
        overrides
      )
  ...
  // ONE -> DTT
  it('swapExactONEForTokensSupportingFeeOnTransferTokens', async () => {
    ...
    await router.swapExactONEForTokensSupportingFeeOnTransferTokens(
      0,
      [wone.address, DTT.address],
      wallet.address,
      MaxUint256,
  ...
  // DTT -> ONE
  it('swapExactTokensForONESupportingFeeOnTransferTokens', async () => {
    const DTTAmount = expandTo18Decimals(5)
      .mul(100)
      .div(99)
    const ONEAmount = expandTo18Decimals(10)
    ...
    await router.swapExactTokensForONESupportingFeeOnTransferTokens(
      swapAmount,
      0,
      [DTT.address, wone.address],
      wallet.address,
...
describe('fee-on-transfer tokens: reloaded', () => {
  ...
  beforeEach(async function() {
    ...
    DTT = await deployContract(wallet, DeflatingHRC20, [expandTo18Decimals(10000)])
    DTT2 = await deployContract(wallet, DeflatingHRC20, [expandTo18Decimals(10000)])
